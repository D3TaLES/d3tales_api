...
class MolOpt(Firework):
    def __init__(self, smiles=None, name="opt_mol", parents=None, priority=None, name_tag='', **kwargs):
        ...
        t = [RunGaussianOpt(smiles=smiles, name=name, name_tag=name_tag, g16_cmd=G16_CMD, proc_vm_key=PROC_VM_KEY,
...
class WTuning(Firework):
    def __init__(self, smiles=None, name="wtuning", parents=None,  priority=None, name_tag='', **kwargs):
        spec = {'_category': CALC_CATEGORY, '_priority': priority} if priority else {'_category': CALC_CATEGORY}
        t = [RunWtuning(smiles=smiles, path=PATH, runfile_log=RUNFILE_LOG, name=name, name_tag=name_tag, geometry="opt_mol", **kwargs)]
        super(WTuning, self).__init__(t, parents=parents, spec=spec, name="{}{}".format(name_tag, name))


class Optimization(Firework):
    def __init__(self, smiles=None, species="groundState", geometry="opt_mol", solvent=None, parents=None, priority=None, name_tag='', **kwargs):
        spec = {'_category': CALC_CATEGORY, '_priority': priority} if priority else {'_category': CALC_CATEGORY}
        _type = "solv_" + solvent if solvent else "gas_phase"
        name = "solv_opt_" + species if solvent else "opt_" + species
        t = [RunGaussianOpt(smiles=smiles, name=name, name_tag=name_tag, g16_cmd=G16_CMD, proc_vm_key=PROC_VM_KEY, path=PATH, runfile_log=RUNFILE_LOG,
...
class Energy(Firework):
    def __init__(self, smiles=None, species="groundState", geometry="groundState", parents=None, priority=None, name_tag='', solvent=None, **kwargs):
        ...
        t = [RunGaussianEnergy(smiles=smiles, name=name, name_tag=name_tag, g16_cmd=G16_CMD, proc_vm_key=PROC_VM_KEY, path=PATH, runfile_log=RUNFILE_LOG,
                               geometry=geometry, type=_type, subtype=species, solvent=solvent, **kwargs)]
        super(Energy, self).__init__(t, parents=parents, spec=spec, name="{}{}".format(name_tag, name))


class TDDFT(Firework):
    def __init__(self, smiles=None, species='groundState', solvent=None, parents=None, priority=None, name_tag='', solv_geom=False, **kwargs):
        spec = {'_category': CALC_CATEGORY, '_priority': priority} if priority else {'_category': CALC_CATEGORY}
        ...
        t = [RunGaussianTDDFT(smiles=smiles, name=name, name_tag=name_tag, path=PATH, runfile_log=RUNFILE_LOG, g16_cmd=G16_CMD, proc_vm_key=PROC_VM_KEY, type='tddft',
